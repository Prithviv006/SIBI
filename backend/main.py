...
def generate_study_plan(topic: str = Form(...), user: str = Form(...)):
    ...
    try:
        ...
        study_plan_content = completion.choices[0].message.content
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to generate study plan: {e}")

    users_collection.update_one(
        {"username": user},
        {"$set": {"study_plan": study_plan_content}}
    )

    return {"message": "Study plan generated and saved!"}

# 🔍 Get User Study Plan
@app.get("/get-user-study")
def get_user_study(user: str):
    user_data = users_collection.find_one({"username": user})
    if user_data and "study_plan" in user_data:
        return {"study_plan": user_data["study_plan"]}
    
    raise HTTPException(status_code=404, detail="Study plan not found for this user.")

# 📝 AI Quiz Generator
@app.get("/generate-quiz")
def generate_quiz(topic: Optional[str] = None):
    if not topic:
        topic = "General Knowledge"
    messages = [
        {"role": "system", "content": "You are an AI quiz generator. Always return exactly 5 multiple-choice questions in the following format. Each question must be on a new line, with options labeled A), B), C), D), and the answer on a new line as 'Answer: X'. Example:\nQ1: What is 2+2?\nA) 3\nB) 4\nC) 5\nD) 6\nAnswer: B\nQ2: ... (repeat for 5 questions)"}
    ]
...
async def analyze_answer_sheet(file: UploadFile = File(...)):
    ...
    # Send extracted text to Groq API (LLaMA 3.3) for e3 t55555555555555555555
    messages = [
